...
<article id="post-<?php the_ID(); ?>" <?php post_class($post_class); ?>>
	<?php if (!is_single()): ?>
		<div class="match-info">
			...
			<a href="<?php the_permalink(); ?>" class="btn"><i
						class="fas fa-play"></i> <?php _e('stream hd', 'stream-sports') ?></a>
		</div>
	...
	<?php if (is_single()): ?>
		<div class="match-header">
			<h2> <?php _e('Watch ', 'stream-sports');
				the_title();
				_e(' Live Stream', 'stream-sports'); ?> </h2>
			<time><?php echo esc_html($new_date); ?> - <?php echo esc_html($new_Time_2); ?></time>
		</div>
		...
		<div class="tab-link-wrapper">
			<nav>
				<div class="nav links-nav" id="nav-tab" role="tablist">
					...
					foreach ($channels_group as $channels_key => $channels_value) {
						...
						?>
						<a class="nav-item nav-link <?php echo esc_attr($channels_active_class); ?>"
						   id="tabs-<?php echo esc_attr($channels_key); ?>" data-toggle="tab"
						   href="#nav-tabs-<?php echo esc_attr($channels_key); ?>" role="tab"
						   aria-controls="nav-tabs-<?php echo esc_attr($channels_key); ?>"
						   aria-selected="<?php (1 == esc_attr($channels_key)) ? 'true' : 'flase'; ?>">
			...
			<div class="tab-content" id="nav-tabContent">
				...
				foreach ($channels_group as $channels_key => $channels_value) {
					...
					?>
					<div class="tab-pane fade <?php echo esc_attr($channels_active_class); ?>"
					     id="nav-tabs-<?php echo esc_attr($channels_key); ?>" role="tabpanel"
					     aria-labelledby="tabs-<?php echo esc_attr($channels_key); ?>">
						<div class="links-content">
							<div class="video-wrapper">
								<div class="play-screen">
									<a href="<?php echo esc_url($channels_value['channel_link']) ?>" class="screen-btn"
									   target="_blank">
								...
								<div class="play-controls align-items-center justify-content-center">
									<div class="play-btn col-1">
										<a href="<?php echo esc_url($channels_value['channel_link']) ?>"
										   class="plying-btn" target="_blank">
											<i class="fas fa-play"></i>
											<i class="fas fa-pause"></i>
										</a>
									</div>
									<div class="play-bar col-10">
										<input type="range" value="0" class="range" min="1" max="100">
									</div>
									<div class="play-zoom col-1">
										<a href="<?php echo esc_url($channels_value['channel_link']) ?>"
										   target="_blank">
		...
		<?php
		if (is_active_sidebar('add-sports')) {
			echo '<div class="mt-3 mb-5">';
			dynamic_sidebar('add-sports');
			echo '</div>';
		}
		?>
